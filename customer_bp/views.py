...
def index():
    ...
    if form.validate_on_submit():
        ...
        if user is None:
            ...
            return redirect(url_for('customer_bp.customer_home', user_name=user['user_name'], email_address=user['email_address']))
...
def change_password():
    ...
    if form.validate_on_submit():
        ...
        if not user:
            ...
            conn.close()
            return redirect(url_for('customer_bp.index'))  # Redirect to login page
...
def register():
    ...
    if form.validate_on_submit():
        ...
        # Check if user exists
        cursor.execute('SELECT * FROM User WHERE email_address = %s or mobile_number = %s', (email, mobile_number))
        user = cursor.fetchone()
        cursor.close()
        ...
        ''', (email, user_name, mobile_number, date_of_birth, password, security_question, security_answer))
...
def book_ticket():
    ...
    if request.method == 'POST':
        ...
        movie_name, theater_name = selected_movie_theater.split('|') 
        ...
        if form.validate_date() == 0:
            flash('Date cannot be less than current date', 'danger')
            return redirect(url_for('customer_bp.book_ticket'))
        elif form.validate_date() == 1:
            flash('Date cannot be more than 3 days', 'danger')
            return redirect(url_for('customer_bp.book_ticket'))
        ...
        cursor.execute('''Update Theater SET seat_capacity = %s where theater_name = %s ''', (new_seat_capacity, theater_name))
        conn.commit()

        conn.close()

        flash("Your booking is confirmed!", "success")
        return redirect(url_for('customer_bp.book_ticket'))  # Redirect to home page after booking
    ...
    return render_template('customer_book.html', form=form, movies_theaters=movies_theaters, user_name = username)

@customer_bp.route('/booking_history', methods=['GET'])
def booking_history():
    ...
    cursor.execute("""SELECT b.booking_id, b.movie_name, b.theater_name, b.date_of_booking, b.time_of_booking, b.no_of_tickets_required, b.total_amount, b.status FROM Booking b WHERE b.email_address = %s ORDER BY b.date_of_booking DESC""", (email_address,))
    ...
    return render_template('customer_history.html', bookings=bookings, username=username)

@customer_bp.route('/cancel_ticket', methods=['GET', 'POST'])
def cancel_ticket():
    ...
    if not email_address:
        ...
        return redirect(url_for('customer_bp.cancel_ticket'))
    ...
    if request.method == 'POST':
        ...
        cursor.execute("""UPDATE Booking SET status = 'Cancelled' WHERE booking_id = %s""", (selected_booking_id,))

        conn.commit()
        conn.close()

        flash("Movie ticket cancellation is done successfully!", "success")
        return redirect(url_for('customer_bp.cancel_ticket'))  # Redirect to home page after cancellation
    ...
    return render_template('customer_cancel.html', username=username, bookings=bookings)